...
useEffect(() => {
      ...
      const updatedPlans = await Promise.all(
        plansData.map(async (plan) => {
          try {
            ...
            // Assign shiftId from buffer rule if exists
            if (bufferRes.data.shiftId) {
              return { ...plan, shiftId: bufferRes.data.shiftId };
            }
            return plan;
          } catch {
  ...
  const handleDelete = async (index) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/admin/plans/${planToDelete.planId}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const validateForm = () => {
    ...
    if (!editorRef.current || !editorRef.current.querySelectorAll('li').length) {
...
const handleSave = async () => {
  ...
  try {
    ...
    if (ruleRes.status === 200 && ruleRes.data) {
      ...
      await fetch(
        `http://localhost:5000/api/plan-shift-buffer-rule/${ruleRes.data.id}`,
        {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
      ...
      await fetch(
        `http://localhost:5000/api/plan-shift-buffer-rule/add`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...
            <div className="modal-body">
              ...
              <div className="form-floating mt-4">
                <input
                  ...
                  onChange={(e) =>
                    setBufferInMinutes(e.target.value)
                  }
                />
